...
def reset_password():
    d = request.json or {}
    email    = d.get('email','').strip().lower()
    otp_code = str(d.get('otp','')).strip()
    new_pw   = d.get('password','')
    if not all([email, otp_code, new_pw]):
    ...
    row = row_to_dict(db.execute(
        ...
        (email,)).fetchone())
    if not row:
        return jsonify({'error': 'No reset request found. Please request a new OTP.'}), 400
    if int(time.time()) > row['expires_at']:
        return jsonify({'error': 'OTP has expired. Please request a new one.'}), 400
    if row['token'] != otp_code:
...
def change_password():
    ...
    if not check_password(current_pw, u['password_hash']):
...
def user_profile():
    ...
    if request.method == 'GET':
        u = row_to_dict(db.execute(
            "SELECT id,name,email,phone,role,department,created_at FROM users WHERE id=?",
            (user['id'],)).fetchone())
        p = row_to_dict(db.execute(
            "SELECT * FROM user_profiles WHERE user_id=?", (user['id'],)).fetchone())
        u['profile'] = p or {}
        u['created_at_ist'] = to_ist(u.get('created_at',''))
        return jsonify({'user': u})
    # PUT / POST — update profile
    d = request.json or {}
    if d.get('name','').strip():
        db.execute("UPDATE users SET name=? WHERE id=?", (d['name'].strip(), user['id']))
    if d.get('phone','').strip():
        db.execute("UPDATE users SET phone=? WHERE id=?", (d['phone'].strip(), user['id']))
    now = ist_now()
    existing = db.execute("SELECT id FROM user_profiles WHERE user_id=?", (user['id'],)).fetchone()
    if existing:
        db.execute(
            "UPDATE user_profiles SET address=?,pincode=?,city=?,bio=?,updated_at=? WHERE user_id=?",
            (d.get('address',''), d.get('pincode',''), d.get('city',''), d.get('bio',''), now, user['id']))
    else:
        db.execute(
            "INSERT INTO user_profiles (user_id,address,pincode,city,bio,updated_at) VALUES (?,?,?,?,?,?)",
            (user['id'], d.get('address',''), d.get('pincode',''), d.get('city',''), d.get('bio',''), now))
    db.commit()
...
def complaint_to_dict(db, c):
    ...
    if (d.get('image_path') or '').startswith('https://'):
